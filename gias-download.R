...
gias.estab.fields <- function(gias_date){
  ...
  gias_establishment_data_clean <- dplyr::select(gias_establishment_data_clean,
                                                 urn,la_code,establishment_number,establishment_name,
                                                 previous_la_code,previous_establishment_number,
                                                 northing,easting,postcode,ukprn,establishment_type_group_name,
                                                 phase_of_education_name,establishment_status_name,
                                                 trusts_name,head_first_name,head_last_name,official_sixth_form_name,
                                                 administrative_ward_name,msoa_name,lsoa_name,uprn,school_capacity,
                                                 gender_name,statutory_low_age,statutory_high_age,open_date,close_date,gias_date)
  gias_establishment_data_clean <- dplyr::mutate(gias_establishment_data_clean,
                                                 laestab = paste(la_code,establishment_number,sep=""),
                                                 previous_laestab = paste(previous_la_code,previous_establishment_number,sep=""),
                                                 heads_name = paste(head_first_name,head_last_name,sep=" ")
                                                 )
  gias_establishment_data_clean <- dplyr::select(gias_establishment_data_clean,
                                                 -c(la_code,establishment_number,head_first_name,head_last_name))
                                                 
  return(gias_establishment_data_clean)                                        
                                                 
}

gias.estab.fields.date.range <- function(gias_date_range,seq_by){
  gias_dates <- seq.Date(as.Date(gias_date_range[1]),as.Date(gias_date_range[2]),by = "month")
  list_of_gias_data <- lapply(gias_dates,function(x) gias.estab.fields(x))
  gias_data_full <- do.call(rbind,list_of_gias_data)
  return(gias_data_full)
}

save_gias_data <- function(gias_date_range,seq_by){
  gias_data <- gias.estab.fields.date.range(gias_date_range,seq_by)
  current_wd <- getwd()
  setwd(
    paste(
      dirname(rstudioapi::getSourceEditorContext()$path),".",sep =""
    )
  )
  write.csv(gias_data,paste("./Data/gias_data",paste(gias_date_range,collapse="_"),"_",seq_by,".csv",sep=""))
  setwd(current_wd)
}

save_gias_data(c("2024-03-01","2024-09-01"),28)
